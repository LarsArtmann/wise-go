...
> **Resolution (2026-09-13 docs-health pass, re-verdicted 2026-09-16):** of the
> f-list, tiers 1–2 shipped through v0.11.0 (quotes, recipients, transfers incl.
> receipts + MT103 payout info, funding, rates, statements, users, MCA/bank
> details, currencies, webhook signature verification, observability, and —
> since v0.11.0 `5a6448c` — webhook subscription CRUD + typed event decoding).
> Still open, routed: addresses, balance-capacity, `Profile.currentState`/
> `externalCustomerId`, OAuth token endpoint, 2026Q4 version header, simulation
> endpoints, batch payments (plan tier 3/4 → ROADMAP); `WithMetrics` (ROADMAP);
> open-banking doc's stale V1 URL fixed 2026-09-13. The companion review
> `docs/reviews/2026-08-08_api-docs-study.md` remains the canonical gap map.
...
| -: | ------------------------------------------------------------------------------------- | :------: | ------------------------------------------------------------------ |
|  ~~1~~ | ~~Re-add BDD test for `WithCorrelationID` header propagation~~ done — correlation-ID coverage via observability + concurrency tests (v0.9.0, 2026-09-13) | ~~P0~~ | ~~`wise_test.go` has `Describe("WithCorrelationID")` that passes~~ |
|  ~~2~~ | ~~Add `WithCorrelationID` to README Features list and Options section~~ done — README observability section | ~~P0~~ | ~~README mentions the option~~ |
|  ~~3~~ | ~~Add `RateLimitError.RateLimitedBy` to README error handling example~~ done — README error example shows rl.RateLimitedBy | ~~P0~~ | ~~README shows `rl.RateLimitedBy` usage~~ |
|  ~~4~~ | ~~Update CHANGELOG `[Unreleased]` with all three features~~ done — sandbox fix + correlation + RateLimitedBy changelogged (v0.8.0-era entries) | ~~P0~~ | ~~CHANGELOG has sandbox fix, correlation ID, rate-limited-by entries~~ |
|  5 | Add godoc example for `WithCorrelationID`                                             |    P1    | `ExampleWithCorrelationID` in `example_test.go`                    |
|  ~~6~~ | ~~Add a test that verifies `setHeaders` sets both Authorization and correlation headers~~ done — shared-client concurrency test asserts header isolation (2026-09-13) | ~~P1~~ | ~~Test exists and passes~~ |
|  ~~7~~ | ~~Add sandbox URL migration note to README for existing users~~ done — AGENTS sandbox-V2 gotcha | ~~P1~~ | ~~README has migration callout~~ |
|  ~~8~~ | ~~Add `GetProfile(ctx, ProfileID)` — simplest single-endpoint addition~~ done — v0.8.0 | ~~P1~~ | ~~Method exists, tested~~ |
|  ~~9~~ | ~~Update open-banking comparison doc sandbox URL (or add note that it's point-in-time)~~ done — fixed 2026-09-13 docs-health pass | ~~P2~~ | ~~Doc no longer misleads~~ |
| 10 | Verify `api.wise-sandbox.com` DNS resolves                                            |    P2    | Verification recorded                                              |
| ~~11~~ | ~~Add exchange rates endpoint (`GET /v1/rates`)~~ done — v0.8.0 | ~~P2~~ | ~~Method exists, tested~~ |
| ~~12~~ | ~~Add Quotes API (`POST /v3/quotes` etc.)~~ done — v0.8.0 | ~~P2~~ | ~~Methods exist, tested~~ |
| ~~13~~ | ~~Add Recipients API (`GET /v2/accounts` etc.)~~ done — v0.8.0 | ~~P2~~ | ~~Methods exist, tested~~ |
| ~~14~~ | ~~Add Transfers API (`GET /v1/transfers/{id}` etc.)~~ done — v0.7.0/v0.8.0 | ~~P2~~ | ~~Methods exist, tested~~ |
| ~~15~~ | ~~Add per-request correlation ID override via context~~ done — v0.9.0 | ~~P2~~ | ~~`WithRequestCorrelationID` or context key~~ |
| ~~16~~ | ~~Add mTLS endpoint support or documentation~~ done — v0.9.0 README mTLS section | ~~P3~~ | ~~`WithMTLS` option or README section~~ |
| ~~17~~ | ~~Add webhook signature verification helper~~ done — v0.9.0 | ~~P3~~ | ~~`VerifyWebhookSignature` exists, tested~~ |
| ~~18~~ | ~~Add Statements format parameter (CSV/PDF)~~ done — v0.9.0 six formats | ~~P3~~ | ~~`GetStatement(ctx, format)` exists~~ |
| ~~19~~ | ~~Add bank account details endpoint~~ done — v0.9.0 | ~~P3~~ | ~~`ListBankDetails(ctx, profileID)` exists~~ |
| ~~20~~ | ~~Add Multi-Currency Account endpoint~~ done — v0.9.0 | ~~P3~~ | ~~`GetMCA(ctx, profileID)` exists~~ |
...
| ~~30~~ | ~~Add `WithLogger` option for structured request logging~~ done — v0.9.0 | ~~P3~~ | ~~Option exists, tested~~ |
...
| ~~34~~ | ~~Add webhook event typed structs~~ done — v0.11.0 ParseWebhookEvent + typed payloads (5a6448c) | ~~P3~~ | ~~Event types parsed~~ |
| 35 | Add batch payment endpoints                                                           |    P3    | Methods exist                                                      |
| 36 | Add address CRUD endpoints                                                            |    P3    | Methods exist                                                      |
| ~~37~~ | ~~Add user endpoints (`GET /v1/me`)~~ done — v0.9.0 | ~~P3~~ | ~~Method exists~~ |
| ~~38~~ | ~~Add delivery estimates endpoint~~ done — v0.8.0 | ~~P3~~ | ~~Method exists~~ |
| ~~39~~ | ~~Add balance creation endpoint (`POST /v4/.../balances`)~~ done — v0.9.0 | ~~P3~~ | ~~Method exists~~ |
| ~~40~~ | ~~Add total funds endpoint (`GET /v1/profiles/{id}/total-funds/{currency}`)~~ done — v0.9.0 | ~~P3~~ | ~~Method exists~~ |
