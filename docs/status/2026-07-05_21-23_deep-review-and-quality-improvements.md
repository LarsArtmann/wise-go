...
| #  | Priority | Task                                                                                                                                                                                                         |
| -- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ~~1~~  | ~~HIGH~~ done — 2026-09-13 — signature is totalCents int64 (8b54f7c) | ~~Replace `classifyTransactionType`'s `amount float64` param with `totalCents int64` ← still open (TODO_LIST P4; signature still `float64` 2026-09-13)~~ |
...
| ~~3~~  | ~~HIGH~~ **Won't implement — declined 2026-09-13 — the six error types implement the Classified interface directly, go-error-family's prescribed path for owned errors.** | ~~Register domain error types with `errorfamily.RegisterClassification` ← still open (TODO_LIST P4)~~ |
...
| ~~6~~  | ~~MED~~ done — 2026-09-13 (8b54f7c) | ~~Extract `wiseDateFormat` constant from `parseWiseDate` ← still open (TODO_LIST P4 micro-batch)~~ |
...
| ~~9~~  | ~~LOW~~ **Won't implement — declined 2026-09-13 — plain string enums; String() would print exactly what fmt already prints.** | ~~Add `fmt.Stringer` implementations for `ProfileType`, `BalanceType`, `TransactionType` ← still open (TODO_LIST P4 micro-batch)~~ |
...
| ~~11~~ | ~~LOW~~ done — 2026-09-13 (8b54f7c, types.go:87) | ~~Add `Profile.UserID` to `ProfileResult` (the raw `Profile.UserID` is currently dropped) ← still open (TODO_LIST P4 micro-batch)~~ |
| ~~12~~ | ~~LOW~~ done — 2026-09-13 (8b54f7c, types.go:91) | ~~Add `Profile.PublicID` to `ProfileResult` (currently dropped) ← still open (TODO_LIST P4 micro-batch)~~ |
...
| ~~16~~ | ~~LOW~~ done — 2026-09-13 (dbab2e4, bench_test.go) | ~~Add benchmarks for hot paths (`Cents()`, `classifyTransactionType`, `mapTransaction`) ← still open (TODO_LIST P4 micro-batch)~~ |
...
| ~~20~~ | ~~LOW~~ done — 2026-09-13 (5f632e7 — errors_test.go + helpers_test.go) | ~~Consider splitting `internal_test.go` by domain (errors_test.go, helpers_test.go) ← still open (P4; partially done — example/sandbox/readme-guard tests already split out)~~ |
| ~~21~~ | ~~LOW~~ done — 2026-09-13 (0fab6f5) | ~~Add `WithUserAgent` option (currently no custom User-Agent header) ← still open (TODO_LIST P4 micro-batch)~~ |
