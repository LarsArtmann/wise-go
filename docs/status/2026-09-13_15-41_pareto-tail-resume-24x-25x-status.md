...
| ~~4~~  | ~~Cut v0.11.0 (webhooks + WithUserAgent + Profile fields) after 1–3 — gorelease pre-verified all-additive~~ done — at 5a6448c (v0.11.0 shipped 2026-09-14) | ~~High~~ | ~~S~~ | ~~Release~~ |
...
| ~~8~~  | ~~docs-health HARVEST: route both reports' §f lists into TODO_LIST/ROADMAP~~ done — 2026-09-16 — this run | ~~High~~ | ~~M~~ | ~~Docs~~ |
| ~~9~~  | ~~Re-verify `docs/releases/v0.10.0-release-notes.md` against the now-known state before publishing~~ **Won't implement — superseded — v0.11.0 shipped; publish Release objects instead (TODO_LIST P1).** | ~~Medium~~ | ~~S~~ | ~~Docs~~ |
| ~~10~~ | ~~Verify the v0.10.0 tag object points at the intended commit (`fe896a8`)~~ done — verified — v0.10.0 points at fe896a8 | ~~Medium~~ | ~~S~~ | ~~Release~~ |
...
| ~~13~~ | ~~Add a "prove the gate can fail" rule to AGENTS.md cross-cutting lessons (from d1)~~ done — pipeline-masking lesson in the global AGENTS cross-cutting lessons | ~~High~~ | ~~S~~ | ~~Docs/Process~~ |
| ~~14~~ | ~~Add "re-verify prior-session facts (tags/releases/branch) at resume" to AGENTS.md lessons (from d2)~~ done — status-reports-stale lesson in the global AGENTS cross-cutting lessons | ~~High~~ | ~~S~~ | ~~Docs/Process~~ |
...
| ~~22~~ | ~~Draft the v0.11.0 release-notes skeleton early (webhooks, WithUserAgent, Profile fields, internal batch)~~ done — at 5a6448c (v0.11.0) | ~~Medium~~ | ~~S~~ | ~~Docs~~ |
...
| ~~30~~ | ~~Godoc example for `VerifyWebhookSignature` + `ParseWebhookEvent` composition (formalizes the declined `VerifyAndParse` helper)~~ done — decided — composition; ParseWebhookEvent godoc documents it | ~~Low~~ | ~~S~~ | ~~Docs~~ |
| 31 | Verify ListProfileWebhookSubscriptions pagination shape against spec (assumed single response)                                                                | Medium   | S      | Quality         |
| ~~32~~ | ~~Replay/idempotency guidance for webhook consumers (dedup on `X-Delivery-Id` — doc the at-least-once semantics)~~ done — README documents X-Delivery-Id dedup + at-least-once semantics | ~~Medium~~ | ~~S~~ | ~~Docs~~ |
...
| ~~39~~ | ~~Sweep FEATURES.md for remaining unaudited numeric claims beyond the two doc-verify guards~~ done — 2026-09-13 — doc-verify count guards cover FEATURES/AGENTS claims | ~~Low~~ | ~~S~~ | ~~Docs~~ |
| ~~40~~ | ~~ROADMAP: fold gorelease's v0.11.0 suggestion into the release-planning note~~ done — at 5a6448c (v0.11.0) | ~~Low~~ | ~~S~~ | ~~Docs~~ |
...
| ~~43~~ | ~~`gitignore`/residue sweep: confirm no dangling `reports/` references in tooling configs~~ done — 2026-09-13 (15-41) — reports/ trashed | ~~Low~~ | ~~S~~ | ~~Cleanup~~ |
...
| ~~47~~ | ~~Add sandbox-live dispatch smoke to the workflow (dispatch-gated, key-gated)~~ done — verified 2026-09-16 — workflow set consistent | ~~Low~~ | ~~S~~ | ~~CI~~ |
